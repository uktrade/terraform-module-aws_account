...
resource "aws_ssm_parameter" "central_log_groups" {
  ...
  value = jsonencode({
    ...
    "dev" : try(var.member.new_logging_platform,false)==true? "arn:aws:logs:eu-west-2:${data.aws_caller_identity.logarchive.account_id}:destination:platform-logging-logstash-distributor-non-production": "arn:aws:logs:eu-west-2:${data.aws_caller_identity.logarchive.account_id}:destination:cwl_log_destination"
  })
